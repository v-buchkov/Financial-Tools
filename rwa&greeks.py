...
YEARS_TILL_MATURITY = 1
# Risk-Free rate in RUB leg
RISK_FREE_RATE_RUB = 0.1425
# Risk-Free rate in CNH leg
RISK_FREE_RATE_CNH = 0.0327
...
# FUNCTIONS
# Call price & Greeks - normalized by strike (to multiply further by RUB_NOTIONAL only)
...
def calculate_rwa(spot, vol_atm, vol_otm, delta_t0, delta_t1, gamma, vega_atm, vega_otm):
    ...
    vega_risk = RUB_NOTIONAL * abs(cbr_constant_for_volatility_change *
                                   100 * (-vega_atm * vol_atm + vega_otm * vol_otm))

    return provision_for_capital * (ocp_risk + gamma_risk + vega_risk)


# START OF THE PROGRAM EXECUTION
# T0 data for the options
greeks_atm = get_call_price_and_greeks(SPOT_START, SPOT_START, YEARS_TILL_MATURITY, SIGMA_ATM)
greeks_50otm = get_call_price_and_greeks(SPOT_START, 1.5 * SPOT_START, YEARS_TILL_MATURITY, SIGMA_OTM)

# Premium collected from the sold call-spread
initial_premium = greeks_atm[0] - greeks_50otm[0]

# Greeks at the time the deal was made
initial_delta = RUB_NOTIONAL * (-greeks_atm[1] + greeks_50otm[1])
initial_gamma = RUB_NOTIONAL * (-greeks_atm[2] + greeks_50otm[2])
initial_vega = RUB_NOTIONAL * (-greeks_atm[3] + greeks_50otm[3])

print('Initial Data:')
print('Value (RUB): {:+,.2f}'.format(RUB_NOTIONAL * (-greeks_atm[0] + greeks_50otm[0])))
print('Delta: {:+,.2f}'.format(initial_delta))
print('Gamma: {:+,.2f}'.format(initial_gamma))
print('Vega: {:+,.2f}'.format(initial_vega))
print('Theta: {:+,.2f}'.format(RUB_NOTIONAL * (-greeks_atm[4] + greeks_50otm[4])))
print('Rho: {:+,.2f}'.format(RUB_NOTIONAL * (-greeks_atm[5] + greeks_50otm[5])))
...
print('Vega risk: {:+,.2f}'.format(RUB_NOTIONAL * 0.25 * 100 *
                                   (-greeks_atm[3] * SIGMA_ATM + greeks_50otm[3] * SIGMA_OTM)))
print('---')
print('RWA: {:+,.2f}'.format(calculate_rwa(SPOT_START, SIGMA_ATM, SIGMA_OTM, initial_delta,
                                           initial_delta, initial_gamma, greeks_atm[3], greeks_50otm[3])))

# Calculate days for Monte Carlo simulation
t = 252 * YEARS_TILL_MATURITY
...
for k in tqdm.tqdm(range(PATHS)):
    ...
    for q in range(1, len(path) - LAST_DAYS_IRRELEVANCE):
        ...
        greeks_50otm = get_call_price_and_greeks(curr_spot, 1.5 * SPOT_START, (t-q)/t, SIGMA_OTM)
        ...
        MtM = RUB_NOTIONAL * (-greeks_atm[0] + greeks_50otm[0] + initial_premium * np.exp(RISK_FREE_RATE_RUB * q / 365))
        position_mc.append(MtM)
        # Get greeks in notional
        greeks_daily = [RUB_NOTIONAL * (-greeks_atm[i] + greeks_50otm[i]) for i in range(1, 6)]
        ...
        RWA_series.append(calculate_rwa(curr_spot, SIGMA_ATM, SIGMA_OTM, -delta_for_hedging / curr_spot,
                                        greeks_daily[0], greeks_daily[1], greeks_atm[3], greeks_50otm[3]))
